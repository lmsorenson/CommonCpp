...
void Read::configure_parser(Parser &parser, shared_ptr<SyntaxNode> syntax_tree, pipeline::Stream<string> &token_stream, pipeline::Stream<Error> &error_queue_) const
{
    ...
    if (b_use_header_line)
        parser.add_subsequence<HeaderToken, ValueToken>("header sequence", parse::SequenceElement::Cardinality::One, err);
        
    parser.add_subsequence<RecordToken, ValueToken>("record sequence", parse::SequenceElement::Cardinality::Many, err);
}
