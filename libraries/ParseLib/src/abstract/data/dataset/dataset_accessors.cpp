 ...
 */
int32_t DataSet::is_descriptor_required(hash::DescriptorID a_descriptor_id) const
{
    ...
    for( auto descriptor : this->logical_data_structure.get_identifier_of_granular_entity() )
    {
        ...
        {
            shared_ptr<Attribute> attribute = dynamic_pointer_cast<Attribute>(descriptor);
...
int32_t DataSet::number_of_entity_instances(hash::EntityID a_entity_id) const
{
    return this->logical_data_structure.get_entity_count(a_entity_id);
}

Model DataSet::get_data_model() const
{
 ...
 */
Instance DataSet::where(hash::KeyInstance a_key_subset, string expected_value) const
{
    ...
    if ( !(keyed_instance = this->get(a_key_subset)).is_valid() )
        return Instance(this, Instance::NULL_INST);
...
Instance DataSet::get(hash::KeyInstance a_descriptor_list) const
{
    ...
    vector<hash::DescriptorInstance> expected_descriptor_buffer = get_descriptors_from_descriptor_id_set(a_descriptor_list, this->logical_data_structure.get_identifier_of_granular_entity());
    ...
    //failure
    : Instance(this, Instance::NULL_INST);
}